...
from django_backend.models import Email, Task, User, ChatMessage, TaskComment, TaskActivity, Tag, Project, Event 
...
@router.get("/email/{email_id}", response_model=List[TaskRead])
def get_tasks_from_email(email_id: int, current_user: User = Depends(get_current_user)):
    """
    Get all tasks linked to an email
    """
    try:
        Email.objects.get(id=email_id)
    except Email.DoesNotExist:
        raise HTTPException(status_code=404, detail="Email not found")

    tasks = Task.objects.filter(email_id=email_id)
